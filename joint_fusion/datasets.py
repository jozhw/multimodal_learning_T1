...
class CustomDatasetOld(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_wsi = [
            self.transforms(Image.open(self.opt.input_wsi_path + tile))
            for tile in tiles
        ]
...
class CustomDatasetDelayed(Dataset):
    def __init__(self, opt, mapping_df, split=None, mode="wsi", train_val_test="train"):
        ...
        Parallel(n_jobs=-1)(
            delayed(self._preprocess_image)(tiles) for tiles in self.mapping_df["tiles"]
        )
        ...
        with h5py.File(self.cache_file, "w") as h5f:
            for idx, sample in self.mapping_df.iterrows():
                ...
                for tile in tiles:
                    ...
                    h5f.create_dataset(
                        tile, data=transformed_image.numpy(), compression="gzip"
                    )
