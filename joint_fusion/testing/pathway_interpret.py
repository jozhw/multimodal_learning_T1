...
Steyaert et al. (2023, Commun Med 3:44): aggregate the model's per-gene integrated
gradients onto MSigDB pathways, rank them, and draw the figures.

Three per-gene signals, written per patient by test.py into IG_6sep/:
  integrated_grads   attribution magnitude          -> ranking (|IG|)
  path_gradients     signed effect on risk          -> direction
  vanilla_gradients  raw endpoint gradient (paper's) -> comparison plot
...
Database-only: gene sets and genes come from MSigDB; lung/NSCLC relevance is flagged
solely by reading MSigDB's own gene-set names (LUNG_NAME_TOKENS), never hand-curated
and never used to rank or test.

Layers:
  A  score each pathway = mean attribution over its member genes; rank by summed |IG|.
...
per-patient and per-gene beeswarms, member-gene table, direction/gradient plots, the
score matrices, pathway_analysis_bundle.npz (for pathway_tests.py), run_metadata.json.
...
# Collections to score: Hallmark (compact headline), Reactome (mechanistic depth, the
# paper's choice), KEGG (canonical + disease maps), C6 (oncogenic signatures).
DEFAULT_COLLECTIONS = (
    "h.all",
    "c2.cp.reactome",
    "c2.cp.kegg_legacy",
...
# Lung/NSCLC flag reads MSigDB's own gene-set names -- no hand-authored biology. In the
# fetched collections this matches KEGG_(NON_)SMALL_CELL_LUNG_CANCER and the C6
# KRAS.*.LUNG.* signatures.
...
def plot_beeswarm(
    ...
    """The paper's Fig. 6 analog: one row per pathway, one dot per patient, x = the
    patient's mean attribution over the pathway's genes, colour = mean expression (z).

    The spread is the point: under a cohort-mean baseline, a pathway that pushes risk up
    in above-average patients and down in below-average ones is a wide colour-split row
    that a bar chart of the mean would collapse to zero.
    """
    import matplotlib
    ...
    for y, j in enumerate(rows):
        values = scores[:, j]
        colour = expression_scores[:, j] if expression_scores is not None else values
        finite = np.isfinite(colour)
        if finite.any() and np.ptp(colour[finite]) > 0:
    ...
    bar.set_label("pathway mean expression (z)", fontsize=8)
    fig.tight_layout()
...
def run(
    ...
    summed_abs = np.abs(scores).sum(axis=0)  # the paper's ranking statistic
    ...
    table = pd.DataFrame(
        {
            ...
            "coverage": coverage,
            # Paper's ranking statistic (size-biased toward small sets; the
            # size-corrected perm_z comes from pathway_tests.py).
            "summed_abs_ig": summed_abs,
            "mean_abs_ig": np.abs(scores).mean(axis=0),
            # Aliases -> path gradient (the primary direction).
            "mean_gradient": mean_path_gradient,
            "direction": [direction_label(g) for g in mean_path_gradient],
    ...
    table = table.sort_values("summed_abs_ig", ascending=False, ignore_index=True)
    table.insert(0, "rank", range(1, len(table) + 1))
    logger.info(
        f"Pathways ranked by summed |IG| (the paper's statistic); median size of "
    ...
    if path_gradient_scores is not None:
        plot_beeswarm(
            ...
            xlabel=(
                ...
                "(per patient; positive = increasing expression raises predicted risk)"
            ),
            title="Pathway sensitivity by path-averaged gradients",
        )
    if vanilla_gradient_scores is not None:
        plot_beeswarm(
            vanilla_gradient_scores[:, order],
            top_pathways,
            expression_top,
            os.path.join(output_dir, "pathway_vanilla_gradient_beeswarm.png"),
            top_n,
            xlabel=(
                "Mean vanilla gradient over pathway genes\n"
                "(per patient endpoint; positive = increasing expression raises predicted risk)"
            ),
            title="Pathway sensitivity by vanilla gradients",
        )
    ...
    metadata = {
        ...
        "analysis_bundle": ANALYSIS_BUNDLE_NAME,
        "attribution": {
            "method": "integrated gradients",
            "baseline": "mean RNA-seq expression over the training split",
            "direction_statistic": "cohort-mean path gradient",
            "local_comparison_statistic": "cohort-mean vanilla gradient",
            "ranking_statistic": "sum over patients of |mean member-gene IG|",
            "note": (
                "The sign of a cohort-mean-baseline IG is patient-relative and cancels "
                "across the cohort; direction is taken from the path gradient, while "
                "vanilla gradients are reported as the Steyaert-style local endpoint "
        ...
        "lung_name_flag": {
            ...
            "note": (
                ...
                "lung/NSCLC (in the fetched collections: "
                "KEGG_(NON_)SMALL_CELL_LUNG_CANCER and the C6 KRAS.*.LUNG.* "
                "signatures). All pathways are still scored against the full "
                "universe; the flag does not choose, rank, or test anything."
            ),
        },
    ...
    log_summary(table, top_n)
    return table


def log_summary(table, top_n):
    logger.info("\n" + "=" * 92)
    logger.info(f"TOP {top_n} PATHWAYS BY SUMMED |IG| (the paper's statistic)")
    logger.info("=" * 92)
    logger.info(f"{'Rank':<5}{'Pathway':<58}{'Sum|IG|':<10}{'Dir':<6}Genes")
    for _, row in table.head(top_n).iterrows():
        ...
        logger.info(
            f"{row['rank']:<5}{name:<58}{row['summed_abs_ig']:<10.4f}{arrow:<6}"
...
def main(config, opt):
    ...
    run(
        ...
        collections=tuple(opt.collections.split(",")),
...
def parse_args():
    ...
    parser.add_argument("--collections", type=str, default=",".join(DEFAULT_COLLECTIONS))
    parser.add_argument("--min-members", type=int, default=10,
                        help="Drop gene sets with fewer measured genes than this.")
