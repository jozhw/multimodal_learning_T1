...
def obtain_gene_info(ensembl_ids, output_dir, chunk_size=1000):
    ...
    if os.path.exists(gene_info_path):
        logger.info(f"Loading existing gene information from {gene_info_path}")
        gene_info_df = pd.read_csv(gene_info_path)
        return gene_info_df

    logger.info(f"Obtaining gene information for {ensembl_len} genes...")
    ...
    for chunk_idx, chunk in enumerate(chunks):
        logger.info(
            f"Processing chunk {chunk_idx + 1}/{len(chunks)} ({len(chunk)} genes)"
        )
        ...
        try:
            ...
            if gene_info is not None and not gene_info.empty:
                ...
                logger.info(f"Warning: No gene info returned for chunk {chunk_idx + 1}")

        except Exception as e:
            logger.exception(f"Error processing chunk {chunk_idx + 1}: {e}")
    ...
    if all_gene_info:
        ...
        logger.info(f"Gene information saved to {gene_info_path}")
...
def load_gene_names_from_mapping(
    ...
    logger.info(f"Found {len(gene_names)} genes in first sample ({first_sample_id})")
    logger.info(f"First 5 genes: {gene_names[:5]}")
    logger.info(f"Last 5 genes: {gene_names[-5:]}")
    ...
    for idx, (sample_id, row) in enumerate(mapping_df.iterrows()):
        if idx % 50 == 0:  # Logger.Info progress every 50 samples
            logger.info(f"  Checking sample {idx + 1}/{len(mapping_df)}: {sample_id}")

        sample_gene_names = list(row["rnaseq_data"].keys())

        if sample_gene_names != gene_names:
            inconsistent_samples.append(sample_id)
            logger.info(f"WARNING: Inconsistent gene names in sample {sample_id}")
            ...
            if len(inconsistent_samples) <= 3:
                ...
                if missing_genes:
                    logger.info(f"  Missing genes: {list(missing_genes)[:5]}...")
                if extra_genes:
                    logger.info(f"  Extra genes: {list(extra_genes)[:5]}...")
    ...
    if inconsistent_samples:
        ...
        if len(inconsistent_samples) > 10:
            logger.info(f"  ... and {len(inconsistent_samples) - 10} more")
        is_consistent = False
    else:
    ...
    # WARNING: This is too slow, better if obtain gene info after the top genes are determined
    # logger.info("\nObtaining gene information...")
    # gene_info_df = obtain_gene_info(gene_names, output_dir)

    return gene_names, is_consistent
...
def verify_hdf5_gene_order(
    ...
    logger.info(
        f"Verifying gene order consistency between HDF5 and original mapping data..."
    )
    ...
    if len(common_sample_ids) == 0:
        logger.info(
            "ERROR: No common sample IDs found between mapping data and HDF5 file"
        )
        return False

    logger.info(f"Found {len(common_sample_ids)} common samples")
    ...
    with h5py.File(h5_file_path, "r") as hdf:
        for sample_id in random_samples:
            logger.info(f"  Checking sample: {sample_id}")
            ...
            if h5_rnaseq is None:
                logger.info(f"    ERROR: Sample {sample_id} not found in HDF5")
            ...
            if mismatches > 0:
                logger.info(f"    ERROR: {mismatches} value mismatches found")
                order_consistent = False
            else:
                logger.info(f"Order and values consistent")
...
def load_integrated_gradients(
    ...
    gene_names, is_consistent = load_gene_names_from_mapping(
        mapping_file_path, output_dir
    )

    if not is_consistent:
        logger.info("WARNING: Gene names are not consistent across samples!")
        logger.info(
    ...
    for file_path in ig_files:
        ...
        # Debug: Logger.Info shape and structure info
        logger.info(
            f"Patient {tcga_id}: Shape = {ig_values.shape}, Type = {type(ig_values)}"
        )
        ...
        logger.info(f"  After processing: Shape = {ig_values.shape}")
        logger.info(
            f"  Value range: [{np.min(ig_values):.6f}, {np.max(ig_values):.6f}]"
        )
        logger.info(f"  Non-zero values: {np.count_nonzero(ig_values)}")
        ...
        if len(ig_values) != len(gene_names):
            ...
            logger.info(
                "This suggests a mismatch between IG calculation and gene order!"
            )
        else:
            logger.info(f"IG dimensions match gene count")

        ig_data[tcga_id] = ig_values

    logger.info(f"\nLoaded IG data for {len(ig_data)} patients")
    logger.info(f"Using {len(gene_names)} gene names")

    return ig_data, gene_names, is_consistent
...
def create_summary_grid_plot(ig_data, patient_ids, top_genes, output_dir):
    ...
    plt.close()
    logger.info(f"Summary grid plot saved to {summary_path}")
...
def create_patient_bar_graphs(
    ...
    for i, patient_id in enumerate(patient_ids):
        ...
        # Debug information
        logger.info(f"  IG values shape: {ig_values.shape}")
        logger.info(f"  Total genes available: {len(ig_values)}")

        available_genes = len(ig_values)
        if available_genes < top_genes:
            logger.info(f"  Adjusting top_genes from {top_genes} to {available_genes}")
        ...
        logger.info(f"  Selected {len(combined_values)} genes for visualization")
        logger.info(
...
def print_gene_rankings(top_positive, top_negative):
    """
    Logger.Info enhanced formatted lists of top positive and negative genes.
    """
    logger.info("\n" + "=" * 160)
    logger.info("TOP 10 MOST POSITIVE GENES ACROSS ALL PATIENTS")
    logger.info("=" * 160)
    logger.info(
    ...
    logger.info("\n" + "=" * 160)
    logger.info("TOP 10 MOST NEGATIVE GENES ACROSS ALL PATIENTS")
    logger.info("=" * 160)
    logger.info(
        f"{'Rank':<6} {'Gene Index':<12} {'Ensembl ID':<18} {'Gene Name':<25} {'Mean IG':<12} {'Std IG':<12} "
        f"{'Count':<8} {'Top10 Freq':<12} {'Avg Pos':<10}"
...
def analyze_gene_consistency(ig_data, gene_names, gene_info_df, top_n=10):
    ...
    logger.info("\n" + "=" * 80)
    logger.info("MOST CONSISTENTLY HIGH-RANKING POSITIVE GENES")
    logger.info("=" * 80)
    ...
    logger.info("\n" + "=" * 80)
    logger.info("MOST CONSISTENTLY HIGH-RANKING NEGATIVE GENES")
    logger.info("=" * 80)
...
def main(config):
    ...
    ig_data, gene_names, gene_info_df, is_consistent = load_integrated_gradients(
        ...
        mapping_file_path=config.data.json_file,
        output_dir=config.testing.output_base_dir,  # Pass output_base_dir for CSV storage
    )

    if not ig_data:
        logger.info(
            "No IG data found. Make sure the IG files exist in ./IG_6sep directory"
        )
        return

    if not is_consistent:
        logger.info("\nWARNING: Gene name inconsistency detected!")
        logger.info("Results may not be reliable. Consider investigating the data.")
    ...
    verify_hdf5_gene_order(
        h5_file_path=config.data.h5_file,
        mapping_file_path=config.data.json_file,
    )

    # Create bar graphs for first 10 patients - NOW USES GENE INFO
    logger.info("Creating bar graphs for gene expression (IG) for 10 patients...")
    ...
    logger.info("Analyzing top genes across all patients...")
    ...
    logger.info("Creating gene pattern heatmaps...")
...
if __name__ == "__main__":
    ...
    output_path = Path(config.testing.output_base_dir) / "interpret_omics"
    ...
    config.gpu.use_multi_gpu = False  # interpreting does not support multigpu currently
